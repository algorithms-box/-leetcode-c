...
// ### 解决思路
// 我们可以使用一个栈来模拟这个过程。栈顶是当前正在检查的陨石。

// 1. **遍历数组**：对数组中的每个陨石进行遍历。
// 2. **处理相遇**：对于每个陨石，检查它是否与栈顶的陨石相遇。
//    - 如果栈为空，直接将陨石压入栈。
//    - 如果栈不为空，根据陨石的方向和大小处理相遇：
//      - **同向**：如果两个陨石同向，它们不会相互影响，将当前陨石压入栈。
//      - **反向**：如果两个陨石反向移动，根据它们的绝对值大小判断：
//        -
//        如果当前陨石的绝对值等于栈顶陨石的绝对值，两者抵消，从栈中移除栈顶陨石。
//        -
//        如果当前陨石的绝对值大于栈顶陨石的绝对值，栈顶陨石被摧毁，继续比较当前陨石与栈中的下一个陨石。
//        -
//        如果当前陨石的绝对值小于栈顶陨石的绝对值，当前陨石被摧毁，继续检查下一个数组元素。
// 3. **结果**：遍历完成后，栈中剩余的陨石就是碰撞后存活的陨石。
...
// - 将 `5` 压入栈。
// - 遇到 `10`，同向（向右），压入栈。
// - 遇到 `-5`，反向（向左），与栈顶的 `10` 相遇：
//   - `10` 大于 `-5`，`-5` 被摧毁，不进入栈。

// 继续这个过程，最终栈中的元素就是碰撞后的结果。
